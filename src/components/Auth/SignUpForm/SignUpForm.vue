  ...
  <AuthFormWrapper
    ...
    :disabled="(
      [
        ...
        v$.password.$error || v$.repeatPassword.$error || !password.value
      ][step] || loading
    )"
  >
    ...
    <label tabindex="0" v-if="step === 1" :class="$style.uploadAvatar">
      <img
        alt="Avatar"
    ...
    >
      <UIInput
        ...
        :error="repeatPassword.error || v$.repeatPassword.$errors[0]?.$message"
      />
    </UIInputWrapper>
    <img
  ...
  const rules = computed(() => ({
    login: {
      ...
      minLength: helpers.withMessage(
        getLocaleError('auth', AuthErrors.INVALID_LOGIN_MIN_LENGTH),
        minLength(3)
      ),
      maxLength: helpers.withMessage(
        getLocaleError('auth', AuthErrors.INVALID_LOGIN_MAX_LENGTH),
        maxLength(12)
      ),
    },
    email: {
    ...
    password: {
      minLength: helpers.withMessage(
        getLocaleError('auth', AuthErrors.INVALID_PASSWORD_MIN_LENGTH),
        minLength(8)
      ),
      sameAs: helpers.withMessage(
        getLocaleError('auth', AuthErrors.PASSWORD_REPEAT_MISMATCH),
